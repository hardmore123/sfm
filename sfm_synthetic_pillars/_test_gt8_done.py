...
# Chamfer 在 P+5cm 时 P→Q 是 5cm，但 Q→P 也是 5cm，平均 = 5cm
# 但 3.856cm 是 22.9% 偏差，奇怪
# 让我看 d_pq 和 d_qp
from scipy.spatial import cKDTree
tree_Q = cKDTree(P_shifted)
tree_P = cKDTree(P)
d_pq, _ = tree_Q.query(P)
d_qp, _ = tree_P.query(P_shifted)
print(f'  d_pq mean: {d_pq.mean():.4f} (期望 0.05)')
print(f'  d_qp mean: {d_qp.mean():.4f} (期望 0.05)')
print(f'  Chamfer (mean): {(d_pq.mean() + d_qp.mean())/2:.4f}')
...
print(f'Volumetric error = {ve:.4f}')
