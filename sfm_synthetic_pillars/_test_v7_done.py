...
test_cfg = C.__class__(
    ...
    scene=C.scene.__class__(
        pillars=[(5.0, 5.0, 0.2, 0.8)],  # 目标在 8m
...
print('image max:', img.max(), 'mean:', img.mean())
...
shadow_db = 20*np.log10(shadow_pixels.mean() + 1e-12) if (sm.sum() > 0 and shadow_pixels.mean() > 0) else -200

print()
print('=== T0.5 验收 (z_s=3, range_max=10) ===')
print('seafloor non-zero: %d (%.1f%%)' % (nz, nz/seafloor_pixels.size*100))
