> # ⚠️ 本文已降级为实现笔记，公式不以此为准
>
> **公式的唯一权威是 `../大论文思想路线/理论修正_T1-T6.md`**（D10 优先级链）。
>
> 本文中的以下公式**已作废**，请勿引用：
> - `σ_Pz ≥ σ_ρ/√(Σsin²φ_k)` 与 `φ_blind = arcsin(σ_ρ/(√N·τ_z))`
>   —— 实为 `1/Λ_zz` 的松界，低估 15 倍（实测 14.89 cm，该式预测 0.95 cm）。
>   正式版依赖**仰角离散度**：`σ_Pz ≈ σ_ρ/(√N·std(φ_k))`，
>   `Δφ_min = σ_ρ/(√N·τ_z)`，`τ_z^crit = √3·σ_ρ/(√N·φ_max)`（ARIS N=10 为 4.2 cm）。
> - 文中混入的阴影反演式 `σ_ρ(z_s−h)²/(D_t·z_s)` 属**另一物理问题**
>   （阴影反演的 σ_h 传播，归 ★II-1 的 σ 校准项），不可与多视可观测性混用。
>
> 本文保留价值：记录实现细节与当时的推导过程，便于追溯。
