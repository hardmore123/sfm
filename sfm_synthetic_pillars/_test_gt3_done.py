...
print(f'Volumetric error(P, P+5cm) = {ve:.4f} (期望 0.7-0.9, 大体一致)')
mean_n, max_n = normal_angle_error(P, normals, P, normals)
print(f'Normal angle error (self) = mean={mean_n:.6f}, max={max_n:.6f} {"PASS" if max_n < 1e-6 else "FAIL"}')
fp = floating_point_density(P, R=0.1)
print(f'Floating point ratio (R=10cm) = {fp:.4f} (期望 < 0.1, 接近 0)')
