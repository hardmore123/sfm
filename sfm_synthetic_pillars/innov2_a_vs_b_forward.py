"""
6.1 A vs B 实验 (forward 模式 - 最欠约束场景)
"""
...
def main():
    # 把 forward 模式数据复制
    src = "./big_paper_sim/forward/input"
    dst = "../BA代码/sim_input_forward"
    if os.path.exists(dst):
        shutil.rmtree(dst)
    shutil.copytree(src, dst)

    input_dir = dst
    gt_dir = "./big_paper_sim/forward/gt"
    inv2_dir = "./big_paper_sim/forward/innovation2"
    ...
    print(f"forward 模式: K={K}/{K_full} 关键帧, M={M} lm, N_obs={N_obs}")
    ...
    print(f"  A: t={t_a:.1f}s, pos={res_a_dict['pose_err_mean_cm']:.2f}cm, "
          f"lm={res_a_dict['lm_err_mean_cm']:.2f}cm, z={res_a_dict['lm_z_err_mean_cm']:.2f}cm")

    print("\n=== B. V2 + 阴影高度先验 (σ=0.5m, w=1) ===")
    ...
    print(f"  B: t={t_b:.1f}s, pos={res_b_dict['pose_err_mean_cm']:.2f}cm, "
          f"lm={res_b_dict['lm_err_mean_cm']:.2f}cm, z={res_b_dict['lm_z_err_mean_cm']:.2f}cm")
    ...
    print("\n" + "=" * 70)
    print("6.1 仰角来源消融（forward 模式 - 最欠约束）")
    print("=" * 70)
    print(f"{'配置':<30} | {'pos (cm)':<10} | {'lm (cm)':<10} | {'z (cm)':<10}")
    print("-" * 70)
    ...
    print("=" * 70)
    with open("./innov2_a_vs_b_forward_result.json", "w") as f:
