# 主档·S5_main_envelope_edge

**目录**：`S5_main_envelope_edge/`  **类别**：`可行`

## 用途

ARIS 主档: ±7.5° / 128 波束 / σ_ρ=1cm / ρ_max=40m. 几何 z_s=4.5, h=2.4, d=16.0, heave=1.2.
...
- 立方: [] m
- 球: [] m
- h_avg = 2.40 m, d_avg = 16.00 m

## 可反演性（T0.9 判据）

- 期望：可反演
- 实测：可反演（匹配）
- h_max = 4.50 m（实际 h_avg = 2.40 m）
- elev_top = -7.5°
- L_s = 18.29 m（被截断：False）
...
- std/mean_nn: 0.694（阈值 0.3）
- 验收: PASS

## 反演精度（T0.7 + T0.8 验收）

- V2 精确式 MAE: 2.384185791015625e-05 cm（验收：≤5 cm 且非零）
